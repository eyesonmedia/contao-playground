...
class ModuleSearch extends \Module
{
    ...
    protected function compile()
    {
        ...
        if ($strKeywords != '' && $strKeywords != '*' && !$this->jumpTo)
        {
            ...
            \System::getContainer()->get('freshframes_search.datacontainer.search')->setSearchData($searchData);
             ...
             * burak aydin
             */
            $titleSameKeyword = array();
            foreach ($arrResult as $subKey => $resultCheck) {
                #var_dump($resultCheck);die;
                if( strtolower($_GET['keywords']) === strtolower($resultCheck['title']) ) {
                    $titleSameKeyword['url'] = $resultCheck['url'];
                    $titleSameKeyword['title'] = $resultCheck['title'];
                    $titleSameKeyword['relevance'] = '100';
                    $titleSameKeyword['filesize'] = $resultCheck['filesize'];
                    $titleSameKeyword['text'] = $resultCheck['text'];
                    $titleSameKeyword['matches'] = $resultCheck['matches'];
                }

                #delete from result
                if(strtolower($resultCheck['title']) == strtolower($_GET['keywords'])){
                    unset($arrResult[$subKey]);
                }

            }
            if(count($titleSameKeyword)) {
                array_unshift($arrResult, $titleSameKeyword);
            }
            ...
            for ($i=($from-1); $i<$to && $i<$count; $i++)
            {
                ...
                foreach ($arrMatches as $strWord)
                {
                     ...
                     * burak aydin
                     */
                    preg_match_all('/(^|\b.{0,200}\PL)' . str_replace('+', '\\+', $strWord) . '(\PL.{0,120}\b|$)/ui', $strText, $arrChunks);
